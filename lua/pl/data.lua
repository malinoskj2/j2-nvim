...
local patterns, function_arg, usplit, array_tostring =
  utils.patterns, utils.function_arg, utils.split, utils.array_tostring
...
function data.read(file, cnfg)
  ...
  if cnfg.numfields then
    for _, n in ipairs(cnfg.numfields) do
      append_conversion(n, try_number)
    end
  end
  ...
  if not cnfg.fieldnames then
    ...
    if not cnfg.convert then
      ...
      for idx, conv in pairs(cnfg.convert) do
        append_conversion(idx, conv)
      end
    end
    if nums == nil then
...
function data.query(data, condn, context, return_row)
  ...
  if condn.sort_by then
    ...
    if sort_var:match "^%$" then
      sort_var = sort_var:sub(2)
    end
