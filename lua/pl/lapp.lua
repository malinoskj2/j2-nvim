...
local function strip(str)
  return lstrip(str):gsub("%s+$", "")
end
local function at(s, k)
  return s:sub(k, k)
end
...
local filetypes = {
  stdin = { io.stdin, "file-in" },
  stdout = { io.stdout, "file-out" },
...
local function xtonumber(s)
  ...
  if not val then
    lapp.error("unable to convert to number: " .. s)
  end
  return val
end
...
function lapp.process_options_string(str, args)
  ...
  for line in lines(str) do
    ...
    if check "-$v{short}, --$o{long} $" or check "-$v{short} $" or check "--$o{long} $" then
      ...
      if res.short and not lapp.slack then
    ...
    if res.rest then
      ...
      if match("$({def} $", line, res) or match("$({def}", line, res) then
        ...
        if ftype == "default" then
          default = true
          if rest == "" then
            lapp.error "value must follow default"
          end
          ...
          elseif not ftype:match "|" then -- plain type
            ...
            constraint = function(s)
              lapp.assert(enump:find("|" .. s .. "|", 1, true), "value '" .. s .. "' not in " .. enums)
            end
          end
        end
        res.rest = rest
  ...
  while i <= #arg do
    ...
    ps = parms[parm]
    if not ps then
      lapp.error("unrecognized parameter: " .. parm)
    end
  ...
  for parm, ps in pairs(parms) do
    if not ps.used then
      if ps.required then
        lapp.error("missing required parameter: " .. parm)
      end
...
setmetatable(lapp, {
  __call = function(tbl, str, args)
    return lapp.process_options_string(str, args)
  end,
})
