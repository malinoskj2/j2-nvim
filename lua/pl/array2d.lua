...
local tonumber, tostring, io, ipairs, string, table = _G.tonumber, _G.tostring, _G.io, _G.ipairs, _G.string, _G.table
local setmetatable, getmetatable = setmetatable, getmetatable

local tablex = require "pl.tablex"
local utils = require "pl.utils"
local types = require "pl.types"
local imap, tmap, reduce, keys, tmap2, tset, index_by =
  tablex.imap, tablex.map, tablex.reduce, tablex.keys, tablex.map2, tablex.set, tablex.index_by
...
function array2d.reduce_cols(f, a)
  assert_arg(1, a, "table")
  return tmap(function(c)
    return reduce(f, column(a, c))
  end, keys(a[1]))
end
...
function array2d.write(t, f, fmt, i1, j1, i2, j2)
  ...
  local rowop
  if fmt then
    rowop = function(row, j)
      fprintf(f, fmt, row[j])
    end
  else
    rowop = function(row, j)
      f:write(tostring(row[j]), " ")
    end
  end
